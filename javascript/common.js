...
$("#create-form,.create-form,.create-form-without-reset").on(
  "submit",
  function (e) {
    e.preventDefault();
    ...
    if (
      $(formElement).hasClass("attendance-table") &&
      $(".search-input").val()
    ) {
      Swal.fire({
        icon: "error",
        text: window.trans["Kindly clear the data from the search field"],
      });
      return 0;
    }
    var school_prefix = "";
    ...
    setTimeout(() => {
      ...
      submitButtonElement
        .val(submitButtonText ? submitButtonText : "Submit")
        .attr("disabled", false);
      formAjaxRequest(
        "POST",
        url,
...
$(".online-admission-form,.create-form-with-captcha").on(
  "submit",
  function (e) {
    e.preventDefault();
    let formElement = $(this);
    ...
    setTimeout(() => {
      ...
      function successCallback(response) {
        ...
        if ($(formElement).hasClass("online-admission-form")) {
          ...
          $(".default-image").attr(
            "src",
            url + "/assets/school/images/Image Preview.png"
          );
        }
...
$(
  "#edit-form,.edit-form,.edit-form-without-reset,.edit-form-staff-payroll-setting"
  ...
  if (status) {
    ...
    Swal.fire({
      icon: "error",
      text: window.trans["Duplicate values are not allowed"],
    });
    submitButtonElement.val(submitButtonText).attr("disabled", false);
  }
});
...
$(document).on("click", ".select-plan", function (e) {
  ...
  Swal.fire({
    ...
    inputPlaceholder:
      '<div class="m-2">' +
      window.trans["I accept the provided"] +
      '<a href="' +
      link +
      '" target="_blank"> ' +
      window.trans["terms_condition"] +
      " </a></div>",
  }).then((result) => {
    if (result.isConfirmed) {
      if (result.value) {
        ...
        Swal.fire({
          icon: "error",
          text: window.trans["please_confirm_terms_condition"],
        });
      }
    }
  });
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      function errorCallback(response) {
        let message =
          window.trans[
            "You have already selected an upcoming billing cycle plan If you want to change your upcoming billing cycle plan please ensure to remove the previously selected plan before making any alterations"
          ];
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      if (result.value) {
        let url =
          baseUrl +
          "/subscriptions/start-immediate-plan/" +
          id +
          "/type/" +
          type;
        let data = null;
...
$(document).on("click", ".add-addon", function (e) {
  ...
  Swal.fire({
    ...
    text: window.trans[
      "This add-on will be added into your current subscribed package and will expire when your current subscription expires"
    ],
    icon: "warning",
    showCancelButton: true,
    confirmButtonColor: "#3085d6",
    ...
    if (result.isConfirmed) {
      if (result.value) {
        let url =
          baseUrl + "/addons/subscribe/" + id + "/" + "package-type/" + type;
        let data = null;
...
$(document).on(
  "click",
  ".no-feature-lock-menu,.no-feature-lock-menu-sub-item",
  function (e) {
    e.preventDefault();
    ...
    if (role == "School Admin") {
      Swal.fire({
        ...
        html:
          '<div class="mb-2">' +
          window.trans["no_permission_upgrade_plan"] +
          '</div><a href="' +
          link +
          '">' +
          window.trans["click_here_to_buy_addon"] +
          "</a>",
      });
...
$(document).ready(function () {
  $(".sidebar .nav-link").each(function (index, value) {
    if (
      typeof $(value).data("access") != "undefined" &&
      !$(value).data("access")
    ) {
