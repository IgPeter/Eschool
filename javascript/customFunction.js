...
function createCkeditor() {
  ...
  // inline editors
  let elements = CKEDITOR.document.find(".equation-editor-inline"),
    i = 0,
    element;
...
const getFeesClassOptionsList = (FeesId, classId) => {
  ...
  if (
    $(FeesId).find('option[data-class-section-id="' + classId + '"]').length
  ) {
    $(FeesId)
      .find('option[data-class-section-id="' + classId + '"]')
      .show()
      .trigger("change");
  } else {
...
const getClassSubjectOptionsList = (SubjectId, classId) => {
  ...
  if ($(SubjectId).find('option[data-class-id="' + classId + '"]').length) {
    $(SubjectId)
      .find('option[data-class-id="' + classId + '"]')
      .show()
      .trigger("change");
  } else {
...
const getFilterSubjectOptionsList = (SubjectId, $this) => {
  ...
  if ($this.val()) {
    if (
      $(SubjectId).find('option[data-class-section="' + $this.val() + '"]')
        .length
    ) {
      $(SubjectId).find('option[value=""]').show();
      $(SubjectId)
        .find('option[data-class-section="' + $this.val() + '"]')
        .show();
    } else {
...
const getExamSubjectOptionsList = (SubjectId, $this, class_section_id) => {
  ...
  $(SubjectId).find("option").hide();
  if (
    $(SubjectId).find('option[data-exam-id="' + $this.val() + '"]').length &&
    $(SubjectId).find(
      'option[data-class-section-id="' + class_section_id + '"]'
    ).length
  ) {
    $(SubjectId)
      .find(
...
const getExamOptionsListByClass = (examId, $this) => {
  ...
  if ($(examId).find('option[data-class-id="' + $this.val() + '"]').length) {
    $(examId)
      .find('option[data-class-id="' + $this.val() + '"]')
      .show();
  } else {
...
const getExamOptionsList = (examId, $this) => {
  ...
  $(examId).find("option").hide();
  if (
    $(examId).find('option[data-session-year="' + $this.val() + '"]').length
  ) {
    $(examId)
      .find('option[data-session-year="' + $this.val() + '"]')
      .show()
      .trigger("change");
  } else {
    $(examId).val("data-not-found").attr("disabled", true).show();
...
const getExamClassOptionsList = (classSection) => {
  ...
  if ($(classSection).find('option[data-class-id="' + class_id + '"]').length) {
    $(classSection)
      .find('option[data-class-id="' + class_id + '"]')
      .show()
      .trigger("change");
  } else {
...
const removeOptionWithAnswer = ($this, deleteElement) => {
  ...
  $("#answer_select")
    .find("option[value = " + optionNumber + "]")
    .remove();
...
function getLastDateOfMonth(month, year) {
  ...
  var monthFormatted = ("0" + (date.getMonth() + 1)).slice(-2); // Adding 1 to get the right month since months are zero-based
  var yearFormatted = date.getFullYear();

  // Format the date as dd-mm-YYYY
  var formattedDate = `${
    day < 10 ? "0" : ""
  }${day}-${monthFormatted}-${yearFormatted}`;
...
function gender_ratio(boys, girls, total_students) {
  ...
  var chart = new ApexCharts(
    document.querySelector("#gender-ratio-chart"),
    options
  );
  chart.render();
}
...
function fees_details(data) {
  ...
  var chart = new ApexCharts(
    document.querySelector("#fees_details_chart"),
    options
  );
  chart.render();
}
...
function subscription_transaction(labels, data) {
  ...
  var chart = new ApexCharts(
    document.querySelector("#subscriptionTransactionChart"),
    options
  );
  chart.render();
}
